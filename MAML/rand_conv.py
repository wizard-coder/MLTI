...
class RandConvModule(nn.Module):
    def __init__(self, net=None, kernel_size=3, in_channels=3, out_channels=3,
        ...
        self.register_buffer('data_mean', None if data_mean is None else torch.tensor(
            data_mean).reshape(3, 1, 1))
        self.register_buffer('data_std', None if data_std is None else torch.tensor(
            data_std).reshape(3, 1, 1))
        ...
        self.register_buffer('range_up', None if not self.clamp_output else (
            torch.ones(3).reshape(3, 1, 1) - self.data_mean) / self.data_std)
        self.register_buffer('range_low', None if not self.clamp_output else (
            torch.zeros(3).reshape(3, 1, 1) - self.data_mean) / self.data_std)
        ...
        # print("Add RandConv layer with kernel size {}, output channel {}".format(
    ...
    def forward(self, input):
        ...
        if not (self.identity_prob > 0 and torch.rand(1) < self.identity_prob):
            ...
            if self.clamp_output:
                output = torch.max(
                    torch.min(output, self.range_up), self.range_low)
        else:
            output = input
...
class RandConv2d(Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, rand_bias=True,
                 ...
                 clamp_output=None, range_up=None, range_low=None, multi_std=None, **kwargs):
        """
        ...
        :param kwargs:
        """
        super(RandConv2d, self).__init__(in_channels=in_channels,
                                         out_channels=out_channels, kernel_size=kernel_size, bias=rand_bias, **kwargs)
        ...
        self.register_buffer(
            'range_up', None if not self.clamp_output else range_up)
        self.register_buffer(
            'range_low', None if not self.clamp_output else range_low)
        if self.clamp_output:
            assert (self.range_up is not None) and (
                self.range_low is not None), "No up/low range given for adjust"

        self.multi_std = multi_std
        ...
        with torch.no_grad():
            if self.distribution == 'kaiming_uniform':
                ...
                if self.multi_std is None:
                    ...
                    scale = np.random.choice(np.array(self.multi_std))
                    std = scale * gain / math.sqrt(fan)
                    with torch.no_grad():
    ...
    def forward(self, input):
        ...
        if self.clamp_output == 'clamp':
            ...
            output_low = torch.min(torch.min(output, dim=3, keepdim=True)[
                                   0], dim=2, keepdim=True)[0]
            output_up = torch.max(torch.max(output, dim=3, keepdim=True)[
                                  0], dim=2, keepdim=True)[0]
            output = (output - output_low)/(output_up-output_low) * \
                (self.range_up-self.range_low) + self.range_low
...
class MultiScaleRandConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes,
                 ...
                 clamp_output=False, range_up=None, range_low=None, multi_std=None, **kwargs
                 ):
        """
        ...
        self.multiscale_rand_convs = nn.ModuleDict(
            {str(kernel_size): RandConv2d(in_channels, out_channels, kernel_size, padding=kernel_size // 2,
                                          ...
                                          clamp_output=self.clamp_output,
                                          range_low=self.range_low, range_up=self.range_up, multi_std=multi_std,
    ...
    def randomize(self):
        self.current_scale = str(
            self.scales[random.randint(0, self.n_scales-1)])
